...
module.exports = {
  ...
  async saveWorkHours(params) {
    ...
    if (_id) {
      ...
      // 查询更新后的完整数据
      const updatedData = await workHoursCollection.doc(_id).get().then(res => res.data[0]);
  ...
  async getWorkHourList(params) {
    ...
    // 使用聚合查询获取关联信息
    const aggregateQuery = db.collection('work_hours')
      .aggregate()
  ...
  async getWorkHourStats(params) {
    ...
    // 使用聚合查询按工人分组统计
    const result = await db.collection('work_hours')
      .aggregate()
